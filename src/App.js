...
// + Diario del día por fecha (localStorage)
// Sin cambios a Google Sheets ni Apps Script
// ============================================================
...
function rowsToEvents(rows) {
  return rows
    ...
    .map((row, i) => ({
      ...
      icon: row[12] || SUBCAT_ICONS[(row[3] || "").toLowerCase().trim()] ||
            CATEGORIES[(row[2] || "actividad").toLowerCase().trim()]?.icon || "📌",
    }))
    .sort((a, b) => {
...
export default function App() {
  ...
  const eventsForDay = useCallback(
    (date) => events.filter(e => e.date === date), [events]
  );
  ...
  const onNoteSaved = () => setNotesTick(t => t + 1);
  ...
  return (
    <div style={{
      ...
      {view === "calendario" && <>
        <DateStrip
          days={days} selectedDate={selectedDate} today={today}
          eventsForDay={eventsForDay} onSelect={setSelectedDate}
          notesTick={notesTick}
        />
        <DayHeader
          selectedDate={selectedDate} isToday={isToday}
          tripDayNumber={tripDayNumber} count={currentEvents.length}
        />
        <EventsList
          events={currentEvents} loading={loading}
      ...
      {showImportant && (
        <ImportantModal
          important={important} loading={loading}
          onClose={() => { setShowImportant(false); setView("calendario"); }}
        />
      )}
      {showAdd && (
        <AddModal
          onClose={() => { setShowAdd(false); setView("calendario"); }}
          onSync={load}
        />
      )}
...
// ═══════════════════════════════════════════════════════════════
// DATE STRIP — con indicador de diario
// ═══════════════════════════════════════════════════════════════
function DateStrip({ days, selectedDate, today, eventsForDay, onSelect, notesTick }) {
  return (
    <div style={{ background:"#fff", borderBottom:"1px solid #EEEEEE" }}>
      <div style={{ display:"flex",gap:6,overflowX:"auto",padding:"14px 18px",scrollbarWidth:"none" }}>
        {days.map(day => {
          ...
          const hasDiary = !!lsGet(diaryKey(day)).trim();
          return (
            <button
              key={day}
              onClick={() => onSelect(day)}
              style={{
                flexShrink:0, width:50, padding:"9px 4px 8px",
                borderRadius:13, border:"none", cursor:"pointer",
                position:"relative",
                background: isSel ? "#0f2027" : isTod ? "#EEF6FF" : "#F7F6F2",
                transition: "all 0.18s",
              }}
            >
              <p style={{ margin:0,fontSize:9,color:isSel?"rgba(255,255,255,0.5)":"#bbb",textTransform:"capitalize" }}>
                {d.toLocaleDateString("es-MX", { weekday:"short" })}
              </p>
              <p style={{ margin:"3px 0 2px",fontSize:19,fontWeight:700,color:isSel?"#fff":isTod?"#2563EB":"#1a1a2e" }}>
                {d.getDate()}
              </p>
              <p style={{ margin:0,fontSize:9,color:isSel?"rgba(255,255,255,0.45)":"#ccc",textTransform:"capitalize" }}>
              ...
              {count > 0 && (
                <div style={{ position:"absolute",bottom:5,left:"50%",transform:"translateX(-50%)",display:"flex",gap:2 }}>
                  {[...Array(Math.min(count, 3))].map((_, i) => (
                    <div key={i} style={{ width:3,height:3,borderRadius:"50%",background:isSel?"#67e8f9":"#0f2027" }}/>
                  ))}
                </div>
              )}
              {/* Punto morado = tiene diario */}
              {hasDiary && (
                <div style={{
                  position:"absolute", top:4, right:5,
                  width:5, height:5, borderRadius:"50%",
                  background: isSel ? "#a78bfa" : "#9333EA",
                }}/>
              )}
            </button>
          );
...
function DayHeader({ selectedDate, isToday, tripDayNumber, count }) {
  return (
    <div style={{ padding:"18px 22px 10px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
      <div>
        <h2 style={{ margin:0,fontSize:20,color:"#1a1a2e",fontWeight:400,textTransform:"capitalize" }}>
          {formatDate(selectedDate)}
        </h2>
        {isToday && <span style={{ fontSize:11,color:"#16A34A",fontWeight:700 }}>◉ HOY</span>}
      </div>
      <div style={{ display:"flex",gap:8 }}>
        <span style={{ background:"#EEEEE8",borderRadius:20,padding:"4px 12px",fontSize:12,color:"#666" }}>
          Día {tripDayNumber}
        </span>
        {count > 0 && (
          <span style={{ background:"#0f2027",borderRadius:20,padding:"4px 10px",fontSize:12,color:"#fff" }}>
...
function EventsList({
  ...
  return (
    <div style={{ padding:"0 18px 0" }}>
      {loading && [1,2,3].map(i => (
        <div key={i} style={{ background:"#fff",borderRadius:18,marginBottom:10,padding:16,border:"1.5px solid #EEEEEE" }}>
      ...
      {events.map(event => (
        <EventCard
          ...
          done={!!checked[event.id]}
          onDone={() => setChecked(prev => ({ ...prev, [event.id]: !prev[event.id] }))}
          showDate={showDate}
          notesTick={notesTick}
...
function EventCard({ event, expanded, onToggle, done, onDone, showDate, notesTick, onNoteSaved }) {
  ...
  useEffect(() => {
    const saved = lsGet(noteKey(event.id, event.date));
    setNoteText(saved);
  }, [notesTick]);
  ...
  return (
    <div
      ...
      style={{
        ...
        marginBottom: 10,
        overflow: "hidden",
        border: `1.5px solid ${expanded ? meta.color : done ? "#86EFAC" : isPending ? "#FCD34D" : "#EEEEEE"}`,
        cursor: "pointer",
        transition: "all 0.2s",
    ...
    >
      ...
      <div style={{ display:"flex",alignItems:"center",padding:"13px 15px",gap:12 }}>
        <div style={{
          width:46,height:46,borderRadius:13,background:meta.bg,
          display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:24,flexShrink:0,opacity:done?0.5:1,
        }}>
          {event.icon}
        </div>
          ...
          <div style={{ display:"flex",gap:8,marginTop:5,flexWrap:"wrap",alignItems:"center" }}>
            {showDate && (
              <span style={{ fontSize:11,color:"#888",fontWeight:600 }}>
                {event.date.slice(8,10)} {new Date(event.date+"T12:00:00").toLocaleDateString("es-MX",{month:"short"})}
              </span>
            )}
            {event.time && <span style={{ fontSize:12,color:"#999" }}>🕐 {event.time}</span>}
            <span style={{
              fontSize:10,fontWeight:700,letterSpacing:0.8,textTransform:"uppercase",
              color:meta.color,background:meta.bg,padding:"2px 9px",borderRadius:20,
            }}>
              {event.subcategory || meta.label}
            </span>
            {isPending && (
              <span style={{ fontSize:9,fontWeight:700,color:"#B45309",background:"#FFFBEB",padding:"2px 7px",borderRadius:20 }}>
                ⚠ PENDIENTE
              </span>
            )}
            {/* Badge de nota personal */}
            {hasNote && (
              <span style={{ fontSize:9,color:"#7C3AED",background:"#F5F3FF",padding:"2px 6px",borderRadius:20,fontWeight:700 }}>
                ✏️ nota
              </span>
            )}
          </div>
        ...
        <span style={{
          fontSize:13,color:expanded?meta.color:"#ddd",
      ...
      {expanded && (
        <div
          style={{ padding:"4px 15px 15px",borderTop:`1px solid ${meta.bg}` }}
        ...
        >
          ...
          {event.cost && event.cost !== "—" && (
            <div style={{ marginTop:9,padding:"8px 12px",background:"#FFF7ED",borderRadius:10 }}>
              <p style={{ margin:0,fontSize:11,color:"#C2410C",fontWeight:600 }}>
                💰 Costo: <span style={{ fontWeight:400,color:"#666" }}>{event.cost}</span>
              </p>
            </div>
          )}
          {event.notes && event.notes !== "—" && (
            <div style={{ marginTop:9,background:"#fff",border:"1px solid #EEEEEE",borderRadius:11,padding:"10px 13px" }}>
          ...
          <div style={{
            ...
            <p style={{ margin:"0 0 7px",fontSize:10,color:"#7C3AED",fontWeight:700,letterSpacing:1,textTransform:"uppercase" }}>
              ✏️ Nota personal
            </p>
            <textarea
              value={noteText}
              ...
              placeholder="Tu recuerdo o nota de esta actividad…"
              rows={3}
              style={{
                width:"100%",
                padding:"9px 11px",
          ...
          <div style={{ display:"flex",gap:8,marginTop:12 }}>
            {event.link && event.link.startsWith("http") && (
              <a
                ...
                onClick={e => e.stopPropagation()}
                style={{
                  flex:1,background:meta.color,color:"#fff",
...
function DiaryCard({ date }) {
  ...
  return (
    <div style={{
      margin: "10px 18px 130px",
      borderRadius: 18,
      background: hasEntry ? "#FEFCE8" : "#fff",
      border: `1.5px solid ${open ? "#F59E0B" : hasEntry ? "#FDE68A" : "#EEEEEE"}`,
      overflow: "hidden",
      transition: "all 0.2s",
      boxShadow: open ? "0 6px 28px rgba(245,158,11,0.18)" : "0 1px 5px rgba(0,0,0,0.04)",
    }}>
      {/* Cabecera del diario */}
      <div
      ...
      >
        <div style={{
          ...
          background: hasEntry ? "#FEF3C7" : "#F7F6F2",
          display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:24,flexShrink:0,
        }}>📔</div>
        <div style={{ flex:1 }}>
          <p style={{ margin:0,fontSize:14,fontWeight:600,color:"#92400E" }}>
            {hasEntry ? "Diario del día" : "Escribe en tu diario"}
          </p>
          {!open && (
            <p style={{ margin:"3px 0 0",fontSize:12,color:"#B45309",lineHeight:1.4 }}>
              {hasEntry
                ? `"${preview}${text.trim().length > 90 ? "…" : ""}"`
                : "¿Qué viviste hoy? Toca para escribir."}
            </p>
          )}
        </div>
        <span style={{
          fontSize:13,color:open?"#F59E0B":"#ddd",
          display:"inline-block",
      ...
      {open && (
        <div style={{ padding:"0 16px 16px" }} onClick={e => e.stopPropagation()}>
          <p style={{ margin:"0 0 8px",fontSize:11,color:"#B45309",fontStyle:"italic" }}>
            {formatDate(date)}
          </p>
          <textarea
            ref={textRef}
            ...
            onBlur={handleSave}
            placeholder="¿Qué fue lo más especial de hoy? ¿Qué comieron, qué vieron, cómo se sintieron? Escribe libremente…"
            rows={8}
            style={{
              width:"100%",
              padding:"13px 14px",
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10 }}>
            <span style={{ fontSize:11,color:"#B45309" }}>
              {wordCount} {wordCount === 1 ? "palabra" : "palabras"} · {text.length} caracteres
            </span>
            <button
    ...
    return (
      <div>
        <div style={{ padding:"18px 22px 10px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          ...
          <span style={{ background:cat.bg,color:cat.dark,borderRadius:20,padding:"4px 10px",fontSize:12,fontWeight:600 }}>
            {list.length}
          </span>
        </div>
        <EventsList
          events={list} loading={loading}
          expandedEvent={expandedEvent} setExpandedEvent={setExpandedEvent}
          checked={checked} setChecked={setChecked}
        ...
        return (
          <button
            ...
            style={{
              ...
              border:"1.5px solid #EEEEEE",borderRadius:18,
              padding:"14px 16px",marginBottom:10,
              cursor:"pointer",display:"flex",alignItems:"center",
              gap:14,textAlign:"left",transition:"all 0.2s",
            }}
          >
            <div style={{
              width:50,height:50,borderRadius:14,background:cat.bg,
              display:"flex",alignItems:"center",justifyContent:"center",
            ...
            <div style={{ flex:1 }}>
              ...
              <p style={{ margin:"3px 0 0",fontSize:12,color:"#999" }}>
                {count} {count === 1 ? "evento" : "eventos"}
                {pend > 0 && <span style={{ color:"#B45309",marginLeft:6 }}>· {pend} pendiente{pend > 1 ? "s" : ""}</span>}
              </p>
            </div>
...
function ImportantModal({ important, loading, onClose }) {
  return (
    ...
    >
      <div
        style={{ background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 22px 44px",width:"100%",maxWidth:480,maxHeight:"82vh",overflowY:"auto" }}
        onClick={e => e.stopPropagation()}
      >
        ...
        <p style={{ margin:"0 0 16px",fontSize:12,color:"#888" }}>
          Editables en la pestaña <strong>Importante</strong> de tu Sheet
        </p>
        ...
        {Object.entries(IMPORTANT_CATS).map(([key, catMeta]) => {
          ...
          return (
            <div key={key} style={{ marginBottom:22 }}>
              <p style={{ fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",color:"#aaa",margin:"0 0 10px" }}>
                {catMeta.label}
              </p>
              {items.map(item => (
                <div key={item.id} style={{ background:catMeta.bg,borderRadius:13,padding:"12px 14px",marginBottom:8 }}>
                  ...
                  {item.link && (
                    <a href={item.link} target="_blank" rel="noreferrer"
                      style={{ display:"inline-block",marginTop:6,fontSize:12,color:"#2563EB",textDecoration:"none",fontWeight:600 }}>
                      Abrir ↗
                    </a>
                  )}
                </div>
...
function AddModal({ onClose, onSync }) {
  ...
  const inp = (label, key, type, placeholder) => (
    <div style={{ marginBottom:12 }}>
      <label style={{ display:"block",fontSize:11,fontWeight:700,color:"#666",marginBottom:4,textTransform:"uppercase",letterSpacing:0.8 }}>
        {label}
      </label>
      <input
  ...
  return (
    ...
    >
      <div
        style={{ background:"#fff",borderRadius:"22px 22px 0 0",padding:"24px 22px 44px",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto" }}
        onClick={e => e.stopPropagation()}
      >
        ...
        <a href={SHEET_EDIT_URL} target="_blank" rel="noreferrer"
          style={{ display:"block",background:"#F7F6F2",borderRadius:10,padding:"8px 12px",marginBottom:16,fontSize:12,color:"#555",textDecoration:"none",textAlign:"center" }}>
          📋 O abre el Sheet directamente ↗
        </a>
        ...
        <div style={{ display:"flex",gap:10,marginBottom:12 }}>
          <div style={{ flex:1 }}>
            ...
            <input type="date" value={form.date} onChange={e => set("date", e.target.value)}
              style={{ width:"100%",padding:"10px 12px",borderRadius:10,border:"1.5px solid #E5E5E0",fontSize:14,color:"#1a1a2e",background:"#FAFAF7",outline:"none" }}/>
          </div>
          <div style={{ width:90 }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
            {Object.entries(CATEGORIES).map(([k,v]) => (
              <button key={k} onClick={() => { set("category",k); set("subcategory",""); }}
                style={{ padding:"6px 12px",borderRadius:20,border:`1.5px solid ${form.category===k?v.color:"#E5E5E0"}`,background:form.category===k?v.bg:"#fff",color:form.category===k?v.color:"#888",fontSize:12,fontWeight:form.category===k?700:400,cursor:"pointer" }}>
                {v.icon} {v.label}
              </button>
            ))}
          </div>
            ...
            <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
              {subcatOptions[form.category].map(s => (
                <button key={s} onClick={() => set("subcategory",s)}
                  style={{ padding:"5px 11px",borderRadius:20,border:`1.5px solid ${form.subcategory===s?"#0f2027":"#E5E5E0"}`,background:form.subcategory===s?"#0f2027":"#fff",color:form.subcategory===s?"#fff":"#666",fontSize:11,cursor:"pointer" }}>
                  {s}
                </button>
              ))}
            </div>
        ...
        {inp("Nº Confirmación / Ticket", "confirmation", "text", "Ej. HTL-12345")}
            ...
            {["confirmado","pendiente"].map(s => (
              <button key={s} onClick={() => set("status",s)}
                style={{
                  ...
                  border:`1.5px solid ${form.status===s?(s==="confirmado"?"#16A34A":"#D97706"):"#E5E5E0"}`,
                  background:form.status===s?(s==="confirmado"?"#F0FDF4":"#FFFBEB"):"#fff",
                  color:form.status===s?(s==="confirmado"?"#16A34A":"#D97706"):"#888",
                  fontWeight:form.status===s?700:400,fontSize:13,cursor:"pointer",
                }}>
        ...
        {saved && (
          <div style={{ background:"#F0FDF4",borderRadius:12,padding:"14px",marginBottom:12,textAlign:"center",border:"1.5px solid #86EFAC" }}>
            ...
            <button onClick={() => { onSync(); onClose(); }}
              style={{ background:"#16A34A",color:"#fff",border:"none",borderRadius:10,padding:"8px 18px",fontSize:13,fontWeight:700,cursor:"pointer" }}>
              Cerrar
            </button>
          </div>
        )}
        ...
        <button onClick={handleSave} disabled={saving}
          style={{ display:"block",width:"100%",background:"#0f2027",color:"#fff",borderRadius:14,padding:"14px 0",fontSize:15,fontWeight:700,border:"none",cursor:"pointer",opacity:saving?0.6:1 }}>
    ...
    <div style={{
      ...
      {tabs.map(tab => {
        ...
        if (tab.external) return (
          <a key={tab.id} href={tab.external} target="_blank" rel="noreferrer"
            style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"5px 10px",textDecoration:"none",opacity:0.5 }}>
